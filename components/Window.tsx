...
export default function Window({
  ...
  // animStyle drives both the open animation and the close transition
  ...
  // Clear open animation after it finishes so transforms don't interfere with drag
  useEffect(() => {
    const t = setTimeout(() => setAnimStyle({}), 270);
    return () => clearTimeout(t);
  }, []);

  // Close animation: shrink + fly toward folder icon
  useEffect(() => {
    ...
    // rAF ensures element is in steady state before we start transition
    requestAnimationFrame(() => {
      setAnimStyle({
        transform: `translate(${dx}px, ${dy}px) scale(0.05)`,
        opacity: 0,
        transition:
          "transform 340ms cubic-bezier(0.4,0,0.2,1), opacity 260ms ease",
        pointerEvents: "none",
      });
    });
  ...
  const onTitleMouseDown = (e: React.MouseEvent) => {
    if (e.button !== 0 || closingStarted.current) return;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="flex items-center gap-2 px-3 py-[9px] bg-[#f5f5f5] border-b border-[#c0c0c0] cursor-grab active:cursor-grabbing select-none shrink-0"
      >
        <button
          onClick={onClose}
